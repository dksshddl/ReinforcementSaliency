...
class CustomEnv(gym.Env):
    ...
    def step(self, action=None):
        ...
        step_idx, lat, lng, start_frame, end_frame = x_data[0], x_data[2], x_data[1], int(x_data[5]), int(x_data[6])
    ...
    def reset(self, video_type="train"):
        ...
        ran_idx = random.randint(0, len(self.x_dict[target_video]) - 1)
        ran_x, ran_y = self.x_dict[target_video][ran_idx], self.y_dict[target_video][ran_idx]

        self.saliency = read_SalMap(self.saliency_info[target_video])
        self.video = []
        cap = cv2.VideoCapture(os.path.join(self.video_path, target_video))
        while True:
            ret, frame = cap.read()
            if ret:
        ...
        for _ in range(n_samples - len(observation)):
            observation = np.concatenate([observation, [embed]])
        assert len(observation) == n_samples
        return observation, y_data, target_video
...
def test(max_epochs=10):
    env = CustomEnv()
    epoch = 0

    while epoch < max_epochs:
        prev_ob, action, target_video = env.reset()
        print("target video is {}, data index is {}".format(target_video[0], target_video[1]))
        while True:
            ob, reward, done, action = env.step(action)
            if done:
                break
            else:
                env.render()
