...
class Trainer(object):
    ...
    def take_action(self, info, env, brain_name, steps, normalize, stochastic=True):
        ...
        if self.is_continuous:
            epsi = np.random.randn(len(info.states), 2)
            feed_dict[self.model.epsilon] = epsi
        if self.use_observations:
            feed_dict[self.model.observation_in] = np.vstack(info.observations)
        if self.use_states:
            feed_dict[self.model.state_in] = info.states
        if self.is_training and self.is_continuous and self.use_states and normalize > 0:
        ...
        next_ob, reward, done, next_ac = env.step(actions)
